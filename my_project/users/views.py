...
db = client['hafsah_tourismdb']

user_collection = db['user']
collection = db['newtourpackages']
categories_collection = db['travelcategories']
...
def register(request):
    if request.method == 'POST':
        ...
        username = data.get('username')
        password = data.get('password')
        profile_photo = data.get('profile_photo', '')  # New field for profile photo
        ...
        user_id = user_collection.insert_one({
            ...
            'country': '',
            'contact_no': '',
            'profile_photo': profile_photo  # Store profile photo URL
        }).inserted_id
...
def book_package(request):
    try:
        data = json.loads(request.body)
        username = data.get("decodedUsername")
...
def get_bookings(request):
    if request.method == 'GET':
        try:
            ...
            # encoded_username = base64.b64encode(username.encode('utf-8')).decode('utf-8')
            bookings = bookings_collection.find({'username': username})
            booking_list = list(bookings)
            ...
            import traceback
            print(traceback.format_exc())  # Print the full traceback to debug more easily
            return JsonResponse({'error': str(e)}, status=500)
